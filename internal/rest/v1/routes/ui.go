...
func RegisterUIRoutes(rg *RouteGroup, app *fiber.App) {
	...
	app.Post("/config/save", func(c *fiber.Ctx) error {
		...
		err := global.UpdateConfig(rg.gctx, func(cfg *config.Config) error {
			...
			if err := validateAutomationInterval("default job", c.FormValue("jobs.sync_interval")); err != nil {
				fieldErrors = append(fieldErrors, err.Error())
			}
			if len(fieldErrors) > 0 {
				return fmt.Errorf("%s", strings.Join(fieldErrors, "; "))
			}

			// All values validated — apply to the candidate configuration.
		...
		if len(fieldErrors) > 0 {
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": strings.Join(fieldErrors, "; "), "fields": fieldErrors})
		}
		if err != nil {
	...
	app.Post("/jobs/config/save", func(c *fiber.Ctx) error {
		...
		err := global.UpdateConfig(rg.gctx, func(cfg *config.Config) error {
			...
			if intervalErr = validateAutomationInterval("default job", cfg.Jobs.SyncInterval); intervalErr != nil {
				return intervalErr
			}
			for _, job := range cfg.GetAllJobs() {
				if intervalErr = validateAutomationInterval("job", job.SyncInterval); intervalErr != nil {
					return intervalErr
				}
			}
			return nil
		...
		if intervalErr != nil {
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": intervalErr.Error()})
		}
		if err != nil {
