...
func RunTrendingMovies(cfg *config.Config, db *database.Database, dryRun bool) {
	...
	if mode == "jellyseerr" {
		runTrendingMoviesJellyseerr(ctx, cfg, db, filteredTrending, scoreMap, dryRun)
	} else {
		runTrendingMoviesDirect(ctx, cfg, db, filteredTrending, scoreMap, dryRun)
	}
}

// runTrendingMoviesDirect adds movies directly to Radarr
func runTrendingMoviesDirect(ctx context.Context, cfg *config.Config, db *database.Database, trendingMovies []integrations.TrendingMovie, scoreMap map[int]struct {
	Score float64
	Rank  int
}, dryRun bool,
) {
		...
		if dryRun {
			...
			if err != nil {
				...
				if strings.Contains(err.Error(), "already") || strings.Contains(err.Error(), "exists") {
					...
					if db != nil {
						scoreInfo := scoreMap[trending.Movie.IDs.TMDB]
...
func runTrendingMoviesJellyseerr(ctx context.Context, cfg *config.Config, db *database.Database, trendingMovies []integrations.TrendingMovie, scoreMap map[int]struct {
	Score float64
	Rank  int
}, dryRun bool,
) {
	...
	for _, trending := range trendingMovies {
		...
		mediaInfo, err := jellyseerrClient.GetMovieInfo(trending.Movie.IDs.TMDB)
		if err != nil {
			log.Debugf("Failed to check Jellyseerr status for '%s (%d)': %v", trending.Movie.Title, trending.Movie.Year, err)
		} else if mediaInfo.HasMediaInfo() {
			log.Debugf("Skipping '%s (%d)' - already requested/available in Jellyseerr", trending.Movie.Title, trending.Movie.Year)
			skipped++
			continue
		}
