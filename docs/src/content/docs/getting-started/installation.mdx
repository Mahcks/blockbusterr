...
If you want one complete walkthrough from installation to your first additions, start with [Your first movie job](/getting-started/quickstart/). This page covers other installation choices and maintaining the container.
...
3. Open `http://localhost:9090` on the Docker host, or `http://YOUR-SERVER-IP:9090` from another device. Confirm the dry-run banner appears.
...
An example of putting Blockbusterr, Radarr, and Sonarr on the same network. Skip this if you already run Radarr and Sonarr. The paths are placeholders; Radarr and Sonarr still need their own indexer, download-client, and media-path configuration.
...
2. Install Go 1.25.13 or newer and a C compiler for SQLite, then download dependencies:
...
**Run from source (restart after Go code changes):**
...
Before turning off dry-run, check which jobs are enabled: interval-based jobs can run when the container starts. Follow [Your first movie job](/getting-started/quickstart/) to configure and preview one small job.
...
Back up the stopped container's actual data mount before major version upgrades. Run from outside the data directory and use a new filename for each backup:
...
docker run --rm \
  ...
  -v "$PWD:/backup" \
  alpine:3.22 tar -czf /backup/blockbusterr-data.tar.gz -C /app/data .
```
</Aside>
