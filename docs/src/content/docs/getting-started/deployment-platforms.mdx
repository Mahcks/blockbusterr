...
| Immutable beta image | `ghcr.io/mahcks/blockbusterr:v2.0.0-beta.5` |
...
2. Set the name to `blockbusterr` and the repository to `ghcr.io/mahcks/blockbusterr:latest-beta`.
...
5. Add `TZ` using your IANA timezone. Add `BLOCKBUSTERR_DRY_RUN=true` while testing the beta.
...
<Steps>
1. Open **Apps**, choose the option to install a custom app, and use `ghcr.io/mahcks/blockbusterr:latest-beta` as the image.
...
4. Add `TZ` and, during beta testing, `BLOCKBUSTERR_DRY_RUN=true` as environment variables.
